...
export const Notification = memo(({ error, success, onClose }) => {
  ...
  return (
    <div className={`fixed top-4 right-4 z-50 p-4 rounded-xl shadow-elevated flex items-center gap-3 max-w-sm animate-slideInRight gpu ${
      ...
      <p className={`text-sm font-medium ${isError ? 'text-red-700' : 'text-emerald-700'}`}>
        {error || success}
      </p>
      <button 
        onClick={onClose} 
        className={`ml-auto p-1 rounded-lg transition-all duration-200 hover:scale-110 active:scale-95 ${
