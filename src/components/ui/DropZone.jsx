    ...
    >
      <div className={`w-16 h-16 mx-auto mb-5 rounded-2xl flex items-center justify-center transition-all duration-300 ${
        dragActive 
          ...
          : 'bg-gradient-to-br from-indigo-100 to-purple-50 group-hover:scale-105'
      }`}>
        <Upload className={`w-7 h-7 transition-all duration-300 ${
          dragActive ? 'text-indigo-600' : 'text-indigo-500 group-hover:text-indigo-600'
        }`} strokeWidth={1.5} />
      </div>
      
      <p className="text-base font-medium text-slate-700 mb-1">Drop files here or click to browse</p>
      <p className="text-sm text-slate-400">Any file type supported</p>
      
      <input ref={fileInputRef} type="file" multiple onChange={handleChange} className="hidden" disabled={disabled} />
      
      <div className="mt-5">
        <span className="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-indigo-500 to-purple-500 text-white rounded-xl text-sm font-medium shadow-lg group-hover:shadow-xl group-hover:scale-105 transition-all duration-300">
