      ...
      {downloadedFiles.length === 0 ? (
        <div className="animate-fadeIn">
          <div className="text-center mb-8">
            <div className="w-16 h-16 bg-gradient-to-br from-indigo-100 to-purple-100 rounded-2xl flex items-center justify-center mx-auto mb-4 animate-float shadow-soft">
          ...
          <div className="max-w-sm mx-auto space-y-4">
            <div className="animate-fadeInUp" style={{ animationDelay: '50ms' }}>
              ...
              <input type="text" value={shareCode} onChange={(e) => setShareCode(e.target.value.toUpperCase())} placeholder="XXXXXX" maxLength="6"
                className="w-full p-4 text-2xl text-center font-bold tracking-[0.3em] font-mono border-2 border-indigo-200 rounded-xl input-modern uppercase placeholder:text-indigo-200 focus:border-indigo-400" />
            </div>
            <div className="animate-fadeInUp" style={{ animationDelay: '100ms' }}>
              ...
              <input type="password" value={downloadPassword} onChange={(e) => setDownloadPassword(e.target.value)} placeholder="Enter password"
                className="w-full p-3 border-2 border-indigo-100 rounded-xl text-sm input-modern" />
            </div>
            <div className="animate-fadeInUp" style={{ animationDelay: '150ms' }}>
              <button onClick={handleReceive} disabled={shareCode.length !== 6 || loading}
                ...
                {loading ? (<><Loader className="w-5 h-5 animate-spin" />Loading...</>) : (<>Access Files<ArrowRight className="w-5 h-5" /></>)}
              </button>
            </div>
          </div>

          <div className="mt-8 p-4 bg-gradient-to-br from-indigo-50/50 to-purple-50/30 rounded-xl animate-fadeInUp border border-indigo-100/50" style={{ animationDelay: '200ms' }}>
            ...
            <ul className="text-sm text-slate-600 space-y-2 stagger-children">
              <li className="flex items-start gap-2">
                <span className="w-5 h-5 bg-gradient-to-br from-indigo-500 to-purple-500 text-white rounded-full flex items-center justify-center text-xs font-medium flex-shrink-0 mt-0.5">1</span>
                Enter the 6-digit code shared with you
              </li>
              <li className="flex items-start gap-2">
                <span className="w-5 h-5 bg-gradient-to-br from-indigo-500 to-purple-500 text-white rounded-full flex items-center justify-center text-xs font-medium flex-shrink-0 mt-0.5">2</span>
                Enter password if the share is protected
              </li>
              <li className="flex items-start gap-2">
                <span className="w-5 h-5 bg-gradient-to-br from-indigo-500 to-purple-500 text-white rounded-full flex items-center justify-center text-xs font-medium flex-shrink-0 mt-0.5">3</span>
                Download your files
              </li>
            </ul>
          </div>
        ...
        <div className="animate-scaleIn">
          ...
          {senderMessage && (
            <div className="mb-5 p-4 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl border border-indigo-100 animate-fadeInUp">
          ...
          <div className="space-y-2 mb-5 stagger-children">
            {downloadedFiles.map((file, index) => (
              <div key={index} className="flex items-center gap-3 p-3 bg-gradient-to-r from-indigo-50/50 to-purple-50/30 rounded-xl hover:from-indigo-50 hover:to-purple-50/50 transition-all duration-200 group hover-lift border border-indigo-100/50">
                ...
                <button onClick={() => downloadFile(file)} className="px-4 py-2 bg-gradient-to-r from-indigo-500 to-purple-500 text-white rounded-lg hover:from-indigo-600 hover:to-purple-600 transition-all duration-200 text-sm font-medium flex items-center gap-1 hover:scale-105 active:scale-95 shadow-lg">
          ...
          <button onClick={resetForm} className="w-full py-3 bg-white text-slate-600 rounded-xl font-medium hover:bg-indigo-50 transition-all duration-200 flex items-center justify-center gap-2 hover-lift shadow-soft border border-indigo-100/50">
            <RefreshCw className="w-4 h-4" />Enter Another Code
          </button>
        </div>
      )}
    </div>
